...
Sostituire `site_love_app` con il ruolo creato nel pannello Neon ed eseguire i
grant con l'utente proprietario:

```sql
grant usage on schema rsvp to site_love_app;
grant select, update on rsvp.households to site_love_app;
grant select on rsvp.invitees to site_love_app;
grant select, insert, update on rsvp.responses to site_love_app;
grant select, insert on rsvp.audit_events to site_love_app;
grant insert on rsvp.admin_events to site_love_app;
