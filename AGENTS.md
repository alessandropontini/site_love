...
`app/page.tsx` is the canonical public-home route. `/rsvp/[token]` is the personalized household response route and `/admin/rsvp` is the spouses-only administration route. The unlinked `/proposal`, `/duomo-proposals`, and `/sun-proposals` routes remain internal visual-review surfaces in development and return 404 in production. `/proposal` is the original Carrara puppet-scene draft and must not be mounted publicly until the couple approves it. The paper-theatre `ExperienceShell`, its four challenges, progression hook, migrations, and saved-state contract remain in the repository for a possible future project, but are not mounted or reachable from this wedding site. No game code or existing browser progress is deleted as part of this separation. Earlier scrollytelling and pixel arcade implementations are likewise unmounted references.
